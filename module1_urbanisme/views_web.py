"""
Vues web pour l'interface utilisateur de CIV-Eye Module 1 - Version Corrigée
"""
...
def dashboard(request):
    ...
    # CORRECTIF A10 : detections_sous_condition (Alerte Orange) compte bien 'sous_condition'
    # et non 'surveillance_preventive' (qui était le compteur orange — erreur sémantique)
    detections_stats = DetectionConstruction.objects.aggregate(
        ...
        detections_sous_condition=Count('id', filter=Q(status='sous_condition')),  # ← Orange
...
def detection_detail(request, detection_id):
    ...
    try:
        ...
        geometry = detection.geometry_geojson if detection.geometry_geojson else ""
        ...
        if detection.zone_cadastrale and detection.zone_cadastrale.geometry_geojson:
            context['zone_geometry'] = detection.zone_cadastrale.geometry_geojson if detection.zone_cadastrale.geometry_geojson else ""
